...
/**
 * Script to create Apple tester accounts
 * Usage: npm run script:create-apple-tester -- "Name" "email" "password"
 */
const createAppleTester = async () => {
    const name = process.argv[2] || 'Apple Tester';
    const email = process.argv[3];
    const password = process.argv[4] || 'AppleTester123!';

    if (!email) {
        console.error('❌ Email is required: npm run script:create-apple-tester -- "Name" "email@example.com" "password"');
        process.exit(1);
    }
    ...
    try {
        ...
        console.log('\n✅ Apple Tester account created successfully!');
        console.log('-------------------------------------------');
